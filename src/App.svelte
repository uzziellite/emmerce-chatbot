<script>
  ...
  /**
   * Handle dynamic updates - FIXED to prevent infinite loops
   */
  $effect(() => {
  ...
  /**
   * Separate effect for WebSocket initialization - runs only when sessionKey changes
   */
  $effect(() => {
    if (sessionKey && !websocketInitialized) {
      const websocketStr = `${emmerceChatbot.ws}?session=${sessionKey}&mode=${emmerceChatbot.debugMode}&nonce=${emmerceChatbot.wsNonce}`;
      ...
      // Setup listeners for the new WebSocket
  ...
  */
  ...
  const submitForm = async (event) => {
    ...
    try {
      // Check to ensure this can not be spammed using other automated tools, like curl etc
      //const apiResponse = await sendRequestToApi(endpoint, `${emmerceChatbot.accessUrl}/waba/send-website-message/${clientId}/`, nonce, 'POST', params);
      ...
      userData = {
        ...
        email,
        phone,
        session_start: apiResponse.data?.created_at,
        session_id: apiResponse.data?.conversation?.session_id,
        session_active: true
      };
      ...
      if (userData.session_id) {
        ...
        // Set sessionKey - this will trigger the WebSocket effect
        sessionKey = userData.session_id;
        websocketInitialized = false; // Allow new WebSocket to be created
        ...
        await manageChatSession(userData.session_id, initialChat);
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if(apiResponse.data?.status === "sent"){
        isSending = false;
        await manageChatSession(sessionKey, data);
  ...
  */
  const terminateChat = async () => {
    ...
    // Store current session key before clearing
    const currentSessionKey = sessionKey;
    
    // STEP 1: Close WebSocket FIRST to stop receiving messages
    // This must happen before anything else to prevent new messages
    if (websocket) {
      websocket.close();
      websocket = null;
    }
    ...
    // STEP 2: Clear sessionKey immediately to prevent $effect from recreating WebSocket
    sessionKey = null;
    chatSessionIsActive = false;
    
    // STEP 3: Wait a moment for WebSocket cleanup to complete
    await new Promise(resolve => setTimeout(resolve, 500));
    
    // STEP 4: Send termination request to server
    try {
      await sendRequestToApi(
        endpoint, 
        `${emmerceChatbot.accessUrl}/waba/website-client-resolve-session/${currentSessionKey}/${clientId}`, 
        nonce,
        'POST'
      );
      
      localStorage.removeItem('user_data');
      await deleteChatSession(currentSessionKey);
      ...
      console.error('Error terminating session on server:', error);
    }
  }
  ...
  onMount(async () => {
     ...
     */
    try {
      //const apiResponse = await sendRequestToApi(endpoint, `${emmerceChatbot.accessUrl}/clients/client-website-settings/${clientId}`, nonce,'GET');
      const apiResponse = await sendRequestToApi(endpoint, `${emmerceChatbot.accessUrl}/start/${clientId}?mode=${emmerceChatbot.debugMode}`, nonce,'GET');
    ...
    */
    ...
    if (localStorage.getItem('user_data')) {
      ...
      if (chatStatus?.status) {
        ...
        firstLoading = false;
        websocketInitialized = false; // Allow WebSocket to be created by effect
      } else {
        showChatStatus = false;
        chatStarted = false;
        await terminateChat();
    ...
    const sessionCheckInterval = setInterval(async () => {
      if (chatSessionIsActive && sessionKey) {
        try {
          const chatStatus = await checkChatSessionValidity(sessionKey);
          const wasActive = chatSessionIsActive;
          chatSessionIsActive = chatStatus?.status;
          
          // Only terminate if status changed from active to inactive
          if (wasActive && !chatSessionIsActive) {
